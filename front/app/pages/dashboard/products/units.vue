...
const getData = async (page = 1, searchParams = {}) => {
  ...
  try {
    ...
    units.value = Array.isArray(data.items) ? data.items : []
...
const openDeleteModal = async (unit, method = '') => {
  ...
  if (method === 'restore') {
    title = 'بازیابی برند'
    text = `آیا می\u200Cخواهید برند "${unit.title}" را بازیابی کنید؟`
    icon = 'question'
    confirmButtonText = 'بله، بازیابی کن'
    confirmButtonColor = '#28a745'
  } else if (method === 'delete') {
    title = 'حذف کامل برند'
    text = `آیا مطمئن هستید که می\u200Cخواهید برند "${unit.title}" را برای همیشه حذف کنید؟ این عمل قابل برگشت نیست!`
    icon = 'error'
    confirmButtonText = 'بله، برای همیشه حذف کن'
    confirmButtonColor = '#dc3545'
  } else {
    title = 'حذف برند'
    text = `آیا می\u200Cخواهید برند "${unit.title}" را حذف کنید؟`
    icon = 'warning'
    confirmButtonText = 'بله، حذف کن'
    confirmButtonColor = '#dc3545'
  }
