<template>
  <div class="supplier-form-page">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h4 class="mb-0">افزودن تأمین‌کننده جدید</h4>
      <NuxtLink to="/accounting/suppliers" class="btn btn-outline-secondary">
        ...
        <div class="card">
          <div class="card-body">
            <form @submit.prevent="submitForm">
              <div class="row g-3">
                <div class="col-md-6">
                  <label for="name" class="form-label">نام تأمین‌کننده <span class="text-danger">*</span></label>
                  <input
                    id="name"
                    v-model="form.name"
                    type="text"
                ...
                <div class="col-md-6">
                  ...
                  <small class="form-text text-muted">
                    مقدار مثبت = تأمین‌کننده بدهکار است، مقدار منفی = تأمین‌کننده بستانکار است
                  </small>
                </div>
              </div>
...
const validateForm = () => {
  ...
  if (!form.name.trim()) {
    errors.value.name = 'نام تأمین‌کننده الزامی است'
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    alert('تأمین‌کننده با موفقیت اضافه شد')
    resetForm()
  } catch (error) {
    ...
    if (error.response?.data?.errors) {
      ...
      alert('خطا در ذخیره تأمین‌کننده')
    }
  } finally {
    loading.value = false
  }
