    ...
    <div class="row mb-4">
      <div class="col-12 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
          <button v-if="selectedTr.length > 0" class="btn btn-outline-danger" @click="bulkDeleteSelected"
            title="حذف گروهی">
      ...
      <div class="card-body">
        <form @submit.prevent="searchFilters">
          <div class="row g-3">
            ...
            <div class="col-md-2">
              <input type="number" class="form-control" placeholder="قیمت از..."
                v-model.number="searchQuery.price_from" />
            </div>
            <div class="col-md-2">
              <input type="number" class="form-control" placeholder="قیمت تا..."
                v-model.number="searchQuery.price_to" />
            </div>
            <div class="col-md-2">
            ...
            <tbody>
              <tr v-for="(product, index) in products" :key="product.id" :class="getRowClass(product.id, index)"
                ...
                <td>
                  ...
                  <small class="text-muted" v-if="product.sell_price">{{ formatPrice(product.sell_price) }} (بدون
                    تخفیف)</small>
                </td>
                <td>
...
const getData = async (iPP, cP, searchParams = {}) => {
  ...
  try {
    ...
    products.value = response.items?.data || []
