  ...
  <div class="product-units-page">
    <div class="row mb-4">
      <div class="col-12 d-flex justify-content-between align-items-center">
        <div>
          <h1 class="fw-bold mb-0">واحدهای اندازگیری</h1>
          <p class="text-muted mt-1">مدیریت واحدهای اندازگیری</p>
    ...
    <div class="row mb-4">
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <form @submit.prevent="searchUnits" class="mb-0" enctype='multipart/form-data'>
              <div class="row g-3">

                <div class="col-md-4">
                  <input type="text" class="form-control" placeholder="جستجو بر اساس عنوان واحد اندازگیری..."
              ...
              <table class="table table-hover mb-0">
                <thead class="table-light">
                  <tr>
                    ...
                    <th width="">عنوان واحد اندازگیری</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(unit, index) in units" :key="unit.id"
                    :class="getRowClass(unit.id, index)" @click="setCurrentRow(index)">
                    <td @click="toggleRowSelectionById(unit.id)">
                      <input type="checkbox" v-if="selectedTr.includes(unit.id)" checked
                        class="form-check-input me-2">
                      <a href="javascript:;" v-else class="text-decoration-none">{{ unit.id }}</a>
                    </td>
                    <td class="text-center">
                      <div style="width: 40px; height: 40px; margin: 0 auto;">
                        <img :src="getUnitIconUrl(unit.id, unit.updated_at)" class="rounded border"
                    ...
                    <td>
                      ...
                      <div class="actionBTN btn-group btn-group-sm float-end">
                        <div v-if="unit.deleted_at">
                          <button class="btn text-warning btn-sm" @click="openDeleteModal(unit, 'restore')"
                            title="بازیافت">
                            <i class="fa fa-refresh"></i>
                          </button>
                          <button class="btn text-danger btn-sm" @click="openDeleteModal(unit, 'delete')"
                            title="حذف برای همیشه">
                            <i class="fa fa-times"></i>
                          </button>
                        </div>
                        <div v-else>
                          <button class="btn text-primary btn-sm" @click="editbrand(unit)" title="ویرایش">
                            <i class="fa fa-pencil"></i>
                          </button>
                          <button class="btn text-danger btn-sm" @click="openDeleteModal(unit)" title="حذف">
                            <i class="fa fa-times"></i>
                          </button>
                        </div>
                      </div>
                  ...
                  <tr v-if="units.length === 0">
                    <td colspan="3" class="text-center py-4 text-muted">
                      ...
                      <div>هیچ واحد اندازگیریی یافت نشد</div>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div class="modal-content">
          <div class="modal-header bg-info text-white">
            <h5 class="modal-title">
              <span v-if="modalMode === 'create'">افزودن واحد اندازگیری جدید</span>
              <span v-else-if="modalMode === 'edit'">ویرایش واحد اندازگیری</span>
              <span v-else-if="modalMode === 'view'">مشاهده واحد اندازگیری</span>
            </h5>
          ...
          <div class="modal-body">
            ...
            <form @submit.prevent="modalMode !== 'view' ? savebrand() : null" v-if="currentbrand">
              <div class="row g-3">
                <div class="col-12">
                  <label class="form-label">نام واحد اندازگیری *</label>
                ...
                <div class="col-12" v-if="modalMode !== 'view'">
                  <label class="form-label">آیکون واحد اندازگیری</label>
                ...
                <div class="col-12" v-if="modalMode === 'view'">
                  ...
                  <div class="border rounded p-2 bg-light">
                    <img :src="getUnitIconUrl(currentbrand.id, currentbrand.updated_at)" class="img-thumbnail"
          ...
          <div class="modal-footer" v-else-if="modalMode !== 'view'">
            ...
            <button type="button" class="btn btn-primary" @click="savebrand">
              <span v-if="modalMode === 'create'">افزودن واحد اندازگیری</span>
              <span v-else-if="modalMode === 'edit'">به روزرسانی واحد اندازگیری</span>
            </button>
          </div>
        </div>
      </div>
...
const getData = async (page = 1, searchParams = {}) => {
  ...
  try {
    let url = `/products/units?limit=${itemsPerPage.value}&page=${page}`
    ...
    error.value = 'خطا در بارگذاری لیست واحدهای اندازگیری'
...
const navigateRows = (direction) => {
  const totalRows = units.value.length
...
const toggleCurrentRowSelection = () => {
  if (currentRowIndex.value === -1 || !units.value[currentRowIndex.value]) return

  const brandId = units.value[currentRowIndex.value].id
...
const performCurrentRowAction = () => {
  ...
  const unit = units.value[currentRowIndex.value]
  editbrand(unit)
}
...
onMounted(() => {
  getData() // بارگذاری واحدهای اندازگیری
...
const bulkDeleteSelected = async () => {
  ...
  const result = await Swal.fire({
    ...
    text: `آیا مطمئن هستید که می‌خواهید ${selectedTr.value.length} واحد اندازگیری انتخاب شده را حذف کنید؟`,
    icon: 'warning',
  ...
  await performBulkDelete(selectedTr, units, () => getData())
}
...
const performBulkDelete = async (selectedItems, brandsArray, refreshCallback) => {
  ...
  for (const brandId of selectedItems.value) {
    try {
      await $api(`/products/units/${brandId}`, {
        method: 'DELETE'
      })
      successCount++
    } catch (err) {
      console.error(`Error deleting unit ${brandId}:`, err)
      errorCount++
    }
  }
  ...
  if (errorCount === 0) {
    await Swal.fire({
      title: 'انجام شد!',
      text: `${successCount} واحد اندازگیری با موفقیت حذف شد.`,
      icon: 'success',
      timer: 2000,
    ...
    await Swal.fire({
      ...
      text: `${successCount} واحد اندازگیری حذف شد. ${errorCount} واحد اندازگیری با خطا مواجه شد.`,
      icon: 'warning',
      customClass: {
        popup: 'swal-rtl'
      }
...
// تابع ساخت URL آیکون واحد اندازگیری
const getUnitIconUrl = (brandId, updated_at = 0) => {
  return `${baseUrl}storage/products/units/${brandId}?v=${updated_at}`
}
...
const savebrand = async () => {
  if (!currentbrand.value.title.trim()) {
    formError.value = 'عنوان واحد اندازگیری الزامی است'
    return
  }
  ...
  try {
    ...
    if (modalMode.value === 'create') {
      response = await $api('/products/units', {
        method: 'POST',
        body: formData
      })
      ...
      response = await $api(`/products/units/${currentbrand.value.id}`, {
...
const openbrandModal = async (mode = 'create', unit = null) => {
...
const editbrand = (unit) => {
  openbrandModal('edit', unit)
}

const viewbrand = (unit) => {
  openbrandModal('view', unit)
}



const openDeleteModal = async (unit, method = '') => {
  let title, text, icon, confirmButtonText, confirmButtonColor

  if (method === 'restore') {
    title = 'بازیابی واحد اندازگیری'
    text = `آیا می خواهید واحد اندازگیری "${unit.title}" را بازیابی کنید؟`
    icon = 'question'
    confirmButtonText = 'بله، بازیابی کن'
    confirmButtonColor = '#28a745'
  } else if (method === 'delete') {
    title = 'حذف کامل واحد اندازگیری'
    text = `آیا مطمئن هستید که می خواهید واحد اندازگیری "${unit.title}" را برای همیشه حذف کنید؟ این عمل قابل برگشت نیست!`
    icon = 'error'
    confirmButtonText = 'بله، برای همیشه حذف کن'
    confirmButtonColor = '#dc3545'
  } else {
    title = 'حذف واحد اندازگیری'
    text = `آیا می خواهید واحد اندازگیری "${unit.title}" را حذف کنید؟`
    icon = 'warning'
    confirmButtonText = 'بله، حذف کن'
    confirmButtonColor = '#dc3545'
  }
  ...
  if (result.isConfirmed) {
    await confirmDelete(unit, method)
  }
}

const confirmDelete = async (unit, method = '') => {
  formloading.value = true
  error.value = null
  ...
  try {
    ...
    if (method === 'restore') {
      url = `/products/units/${unit.id}/restore`
      httpMethod = 'PATCH'
      successMessage = 'واحد اندازگیری با موفقیت بازیابی شد!'
    } else if (method === 'delete') {
      url = `/products/units/${unit.id}/force`
      httpMethod = 'DELETE'
      successMessage = 'واحد اندازگیری برای همیشه حذف شد!'
    } else {
      url = `/products/units/${unit.id}`
      httpMethod = 'DELETE'
      successMessage = 'واحد اندازگیری با موفقیت حذف شد!'
    }
    ...
    // حذف از لیست
    const filtered = units.value.filter(item => item.id !== unit.id)
    ...
    if (status === 404) {
      errorMessage = 'واحد اندازگیری یافت نشد'
    } else if (status === 403) {
