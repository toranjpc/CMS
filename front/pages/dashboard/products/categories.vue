              ...
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  <tr v-for="(category, index) in categories1" :key="category.id"
                    :class="getRowClass1(category.id, index)" @click="setCurrentRow(0, index)">
                    <td @click="toggleRowSelection1(category.id)">
                      <input type="checkbox" checked v-if="selectedTr1.includes(category.id)" class="form-check-input">
                      <span v-else>{{ category.id }}</span>
                    </td>
              ...
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  <tr v-for="(category, index) in categories2" :key="category.id"
                    :class="getRowClass2(category.id, index)" @click="setCurrentRow(1, index)">
                    <td @click="toggleRowSelection2(category.id)">
                      <input type="checkbox" checked v-if="selectedTr2.includes(category.id)" class="form-check-input">
                      <span v-else>{{ category.id }}</span>
                    </td>
                    <td>
              ...
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  <tr v-for="(category, index) in categories3" :key="category.id"
                    :class="getRowClass3(category.id, index)" @click="setCurrentRow(2, index)">
                    <td @click="toggleRowSelection3(category.id)">
                      <input type="checkbox" checked v-if="selectedTr3.includes(category.id)" class="form-check-input">
                      <span v-else>{{ category.id }}</span>
                    </td>
                    <td>
                      {{ category.title }}
...
const getData1 = async (page = 1, search = '') => {
  ...
  try {
    let url = `/products/categories?limit=${itemsPerPage.value}&page=${page}`
    if (search.trim()) url += `&title=${encodeURIComponent(search)}`

    console.log('Requesting URL:', url)
    const response = await $api(url)
    console.log('API Response for level 1:', response?.items)

    // اطمینان از اینکه داده‌ها همیشه آرایه هستند
    const data = response?.items || {}
    categories1.value = Array.isArray(data.data) ? data.data : []
...
const getData2 = async (parentId, page = 1, search = '') => {
  ...
  try {
    let url = `/products/categories?limit=${itemsPerPage.value}&page=${page}&father=${parentId}`
    if (search.trim()) url += `&title=${encodeURIComponent(search)}`
    ...
    // اطمینان از اینکه داده‌ها همیشه آرایه هستند
    const data = response?.items || {}
    categories2.value = Array.isArray(data.data) ? data.data : []
...
const getData3 = async (parentId, page = 1, search = '') => {
  ...
  try {
    ...
    // اطمینان از اینکه داده‌ها همیشه آرایه هستند
    const data = response?.items || {}
    categories3.value = Array.isArray(data.data) ? data.data : []
...
const savecategory = async () => {
  ...
  try {
    ...
    if (currentcategory.value.parent_id) {
      formData.append('parent_id', currentcategory.value.parent_id || null)
    }
    ...
    if (modalMode.value === 'create') {
      response = await $api('/products/categories', {
        method: 'POST',
        body: formData
      })
      ...
      response = await $api(`/products/categories/${currentcategory.value.id}`, {
        method: 'POST',
        body: formData,
      ...
      if (response?.data) {
        ...
        if (!updatedCategory.f_id || updatedCategory.f_id == 0) {
          const index = categories1.value.findIndex(cat => cat.id === updatedCategory.id)
          if (index !== -1) {
            categories1.value[index] = updatedCategory
          }
        } else {
          const parentLevel = getCategoryLevel(updatedCategory.f_id)
          if (parentLevel === 1) {
            const index2 = categories2.value.findIndex(cat => cat.id === updatedCategory.id)
            if (index2 !== -1) {
              categories2.value[index2] = updatedCategory
            }
          } else if (parentLevel === 2) {
            const index3 = categories3.value.findIndex(cat => cat.id === updatedCategory.id)
            if (index3 !== -1) {
              categories3.value[index3] = updatedCategory
            }
          }
        }
...
const confirmDelete = async (category, method = '') => {
  ...
  try {
    ...
    if (method === 'restore') {
      url = `/products/categories/${category.id}/restore`
      httpMethod = 'PATCH'
      successMessage = 'دسته بندی با موفقیت بازیابی شد!'
    } else if (method === 'delete') {
      url = `/products/categories/${category.id}/force`
      httpMethod = 'DELETE'
...
const getcategoryIconUrl = (categoryId, updated_at = 0) => {
  return `${baseUrl}storage/products/categories/${categoryId}?v=${updated_at}`
}
