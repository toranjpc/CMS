  ...
  <div class="user-categorys-page">
    <div class="row mb-4">
      <div class="col-12 d-flex justify-content-between align-items-center">
        <div>
          <h1 class="fw-bold mb-0">ویژگی محصولات</h1>
          <p class="text-muted mt-1">مدیریت ویژگی ها و سطح دسترسیهای محصولات سیستم</p>
          ...
          <div class="card-body">
            <form @submit.prevent="searchCategories" class="mb-0" enctype='multipart/form-data'>
              <div class="row g-3">
                ...
                <div class="col-md-4">
                  <input type="text" class="form-control" placeholder="جستجو بر اساس عنوان ویژگی..."
    ...
    <div class="modal fade" :class="{ show: showcategoryModal }"
      ...
      <div class="modal-dialog modal-lg">
        <form @submit.prevent="savecategory()" v-if="currentcategory">
          <div class="modal-content">
            <div class="modal-header bg-info text-white">
              <h5 class="modal-title">
                <span v-if="modalMode === 'create'">افزودن ویژگی جدید</span>
                <span v-else-if="modalMode === 'edit'">ویرایش ویژگی</span>
                <span v-else-if="modalMode === 'view'">مشاهده ویژگی</span>
              </h5>
              <button type="button" class="btn-close"
                @click="showcategoryModal = false; currentcategory = null"></button>
            </div>
            <div class="modal-body">
              ...
              <div class="row g-3">
                ...
                <div class="col-12">
                  ...
                  <input type="text" class="form-control" v-model="currentcategory.values"
                    :readonly="modalMode === 'view'" ref="categoryvaluesInput"  />
                </div>
                <div class="col-12" v-if="modalMode !== 'view'">
                  <label class="form-label">آیکون ویژگی</label>
            ...
            <div class="modal-footer" v-else-if="modalMode !== 'view'">
              ...
              <button type="submit" class="btn btn-primary">
                <span v-if="modalMode === 'create'">افزودن ویژگی</span>
                <span v-else-if="modalMode === 'edit'">به روزرسانی ویژگی</span>
              </button>
            </div>
          </div>
        </form>
...
const getData = async (page = 1, searchParams = {}) => {
  ...
  try {
    let url = `/products/features?limit=${itemsPerPage.value}&page=${page}`
    ...
    // اطمینان از اینکه دادهها همیشه آرایه هستند
    ...
    // در صورت خطا، آرایهها را خالی کنیم
    categories.value = []
    totalItems.value = 0
    totalPages.value = 0
    currentPage.value = 1
...
const performBulkDelete = async (selectedItems, categoriesArray, refreshCallback) => {
  ...
  for (const categoryId of selectedItems.value) {
    try {
      await $api(`/products/features/${categoryId}`, {
        method: 'DELETE'
      })
      successCount++
    } catch (err) {
...
const handleIconUpload = (event) => {
  ...
  if (file) {
    ...
    if (!allowedTypes.includes(file.type) && !file.name.toLowerCase().endsWith('.svg')) {
      formError.value = 'فرمت فایل مجاز نیست. لطفا از فرمتهای JPG, PNG, GIF, WebP یا SVG استفاده کنید'
      event.target.value = ''
      return
    }
...
const getCategoryIconUrl = (categoryId, updated_at = 0) => {
  return `${baseUrl}storage/products/features/${categoryId}?v=${updated_at}`
}
...
const savecategory = async () => {
  if (!currentcategory.value.title.trim()) {
    formError.value = 'عنوان ویژگی الزامی است'
    return
  }
  ...
  try {
    ...
    formData.append('values', currentcategory.value.values)
    ...
    if (modalMode.value === 'create') {
      response = await $api('/products/features', {
        method: 'POST',
        body: formData
      })
      ...
      response = await $api(`/products/features/${currentcategory.value.id}`, {
...
const openDeleteModal = async (category, method = '') => {
  ...
  if (method === 'restore') {
    title = 'بازیابی ویژگی'
    text = `آیا میخواهید ویژگی "${category.title}" را بازیابی کنید؟`
    icon = 'question'
    confirmButtonText = 'بله، بازیابی کن'
    confirmButtonColor = '#28a745'
  } else if (method === 'delete') {
    title = 'حذف کامل ویژگی'
    text = `آیا مطمئن هستید که میخواهید ویژگی "${category.title}" را برای همیشه حذف کنید؟ این عمل قابل برگشت نیست!`
    icon = 'error'
    confirmButtonText = 'بله، برای همیشه حذف کن'
    confirmButtonColor = '#dc3545'
  } else {
    title = 'حذف ویژگی'
    text = `آیا میخواهید ویژگی "${category.title}" را حذف کنید؟`
    icon = 'warning'
    confirmButtonText = 'بله، حذف کن'
    confirmButtonColor = '#dc3545'
  }
...
const confirmDelete = async (category, method = '') => {
  ...
  try {
    ...
    if (method === 'restore') {
      url = `/products/features/${category.id}/restore`
      httpMethod = 'PATCH'
      successMessage = 'ویژگی با موفقیت بازیابی شد!'
    } else if (method === 'delete') {
      url = `/products/features/${category.id}/force`
      httpMethod = 'DELETE'
      successMessage = 'ویژگی برای همیشه حذف شد!'
    } else {
      url = `/products/features/${category.id}`
      httpMethod = 'DELETE'
