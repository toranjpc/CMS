...
const saveUsedPrice = (productId, price) => {
  ...
  if (process.client) {
    localStorage.setItem('buy_product_prices', JSON.stringify(productPrices.value))
  }
}

// Load saved prices from localStorage
if (process.client) {
  const saved = localStorage.getItem('buy_product_prices')
  if (saved) {
...
const submit = async () => {
  ...
  try {
    ...
    //   icon: 'success',
    //   title: 'ثبت موفق',
    //   text: loadedInvoice.value?.id ? 'فاکتور خرید با موفقیت ویرایش شد' : 'فاکتور خرید با موفقیت ثبت شد'
    // })
  } catch (error) {
